...
<section class="flex justify-center items-center mt-20">
  <div class="bg-white p-8 rounded-lg shadow-md w-full md:w-600 mx-6">
    ...
    <form method="POST" action="/listings/<?= $listing->id ?>">
      ...
      <div class="message bg-red-100 my-3 py-2 px-2">
        <?= $error ?>
      </div>
      <?php endforeach; ?>
      <?php endif; ?>
      ...
      <div class="mb-4">
        <textarea name="description" value="<?= $listing->description ?>"
          placeholder="Job Description"
          class="w-full px-4 py-2 border rounded focus:outline-none"><?= $listing->description ?></textarea>
      </div>
      <div class="mb-4">
        <input type="text" name="salary" value="<?= $listing->salary ?? '' ?>"
          placeholder="Annual Salary"
          class="w-full px-4 py-2 border rounded focus:outline-none" />
      </div>
      <div class="mb-4">
        <input type="text" name="requirements"
          value="<?= $listing->requirements ?? '' ?>" placeholder="Requirements"
          class="w-full px-4 py-2 border rounded focus:outline-none" />
      </div>
      <div class="mb-4">
        <input type="text" name="benefits"
          value="<?= $listing->benefits ?? '' ?>" placeholder="Benefits"
          class="w-full px-4 py-2 border rounded focus:outline-none" />
      </div>
      <div class="mb-4">
